...
def test_build_agent_executor_falls_back_when_model_has_no_tools(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from backend.src.slothbearflow_backend.agent.agent_executor import BasicChatExecutor, build_agent_executor
    from backend.src.slothbearflow_backend.config import get_settings
...
def test_get_chat_llm_uses_ollama_provider(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from backend.src.slothbearflow_backend.config import get_settings
    from backend.src.slothbearflow_backend.llm import get_chat_llm
    ...
    monkeypatch.setattr("backend.src.slothbearflow_backend.llm.build_ollama_chat_model", lambda **kwargs: FakeChatOllama(**kwargs))
    get_settings.cache_clear()
...
def test_get_chat_llm_uses_openai_provider(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("backend.src.slothbearflow_backend.llm.build_openai_chat_model", lambda **kwargs: FakeChatOpenAI(**kwargs))
    get_settings.cache_clear()
...
def test_get_embedding_function_uses_openai_provider(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "backend.src.slothbearflow_backend.rag.embedding.build_openai_embeddings",
...
def test_chat_streams_when_enabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("backend.src.slothbearflow_backend.main.build_agent_executor", lambda **kwargs: FakeStreamExecutor())
...
def test_chat_works_with_in_memory_session_store(monkeypatch: pytest.MonkeyPatch) -> None:
    from backend.src.slothbearflow_backend.main import app
    from backend.src.slothbearflow_backend.output_schema import ChatOutput
    ...
    monkeypatch.setattr("backend.src.slothbearflow_backend.main.get_vector_store", lambda settings=None: None)
    monkeypatch.setattr(
        "backend.src.slothbearflow_backend.main.structured_chat_output_from_text",
...
def test_ingest_accepts_job_when_rag_enabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("backend.src.slothbearflow_backend.main.worker_loop", fake_worker_loop)
    get_settings.cache_clear()
    with TestClient(app) as client:
        r = client.post("/ingest", json={"source": "a.txt", "text": "hello"})
...
def test_chat_returns_rag_citations(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("backend.src.slothbearflow_backend.main.build_agent_executor", lambda **kwargs: FakeExecutor())
    monkeypatch.setattr("backend.src.slothbearflow_backend.main.get_vector_store", lambda settings=None: object())
    monkeypatch.setattr("backend.src.slothbearflow_backend.main.get_last_rag_sources", lambda: ["refund-policy.md"])
    monkeypatch.setattr(
        "backend.src.slothbearflow_backend.main.get_last_rag_citations",
